...
from blackscholes import BlackScholesIronCondorLong, BlackScholesIronCondorShort, Black76IronCondorLong, Black76IronCondorShort
...
class TestBlack76IronCondorLong:
    def test_init(self):
        # Assert K1 < K2 < K3 < K4
        with pytest.raises(AssertionError):
    ...
    def test_individual_methods(self):
        iron_condor = Black76IronCondorLong(
            test_F, test_K1, test_K2, test_K3, test_K4, test_T, test_r, test_sigma
        )
        test_methods = list(iron_condor.call1.get_all_greeks().keys()) + [
            "price",
        ]
...
class TestBlack76IronCondorShort:
    def test_init(self):
        # Assert K1 < K2 < K3 < K4
        with pytest.raises(AssertionError):
    ...
    def test_individual_methods(self):
        iron_condor = Black76IronCondorShort(
            test_F, test_K1, test_K2, test_K3, test_K4, test_T, test_r, test_sigma
        )
        test_methods = list(iron_condor.call1.get_all_greeks().keys()) + [
            "price",

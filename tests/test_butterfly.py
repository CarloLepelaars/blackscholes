import pytest

from blackscholes import BlackScholesButterflyLong, BlackScholesButterflyShort

from tests.helpers import MARKET_3K, assert_rejects, assert_structure

_BAD_WINGS = (dict(K1=60, K2=50, K3=40), dict(K1=41, K2=40, K3=50), dict(K1=41, K2=50, K3=60))


@pytest.mark.parametrize("cls", [BlackScholesButterflyLong, BlackScholesButterflyShort])
def test_init_requires_symmetric_strikes(cls):
    assert_rejects(cls, MARKET_3K, *_BAD_WINGS)


class TestBlackScholesButterflyLong:
    def test_individual_methods(self):
        butterfly = BlackScholesButterflyLong(**MARKET_3K)
        # Long (call) butterfly = Call1 - 2 * Call2 + Call3
        assert_structure(butterfly, lambda s, a: getattr(s.call1, a)() - 2 * getattr(s.call2, a)() + getattr(s.call3, a)())


class TestBlackScholesButterflyShort:
    def test_individual_methods(self):
        butterfly = BlackScholesButterflyShort(**MARKET_3K)
        # Short (put) butterfly = -Put1 + 2 * Put2 - Put3
        assert_structure(butterfly, lambda s, a: -getattr(s.put1, a)() + 2 * getattr(s.put2, a)() - getattr(s.put3, a)())

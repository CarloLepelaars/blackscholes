...
from blackscholes import BlackScholesButterflyLong, BlackScholesButterflyShort, Black76ButterflyLong,Black76ButterflyShort

# Test parameters
test_S = 55.0  # Asset price of 55
test_F = 55.0  # Asset price of 55
...
class TestBlack76ButterflyLong:
    def test_init(self):
        # Assert K1 < K2 < K3
        with pytest.raises(AssertionError):
            Black76ButterflyLong(
                F=test_F, K1=60, K2=50, K3=40, T=test_T, r=test_r, sigma=test_sigma
            )
        with pytest.raises(AssertionError):
            Black76ButterflyLong(
                F=test_F, K1=41, K2=40, K3=50, T=test_T, r=test_r, sigma=test_sigma
            )
        # Assert K2 - K1 = K3 - K2 (symmetry)
        with pytest.raises(AssertionError):
            Black76ButterflyLong(
                F=test_F, K1=41, K2=50, K3=60, T=test_T, r=test_r, sigma=test_sigma
            )

    def test_individual_methods(self):
        butterfly = Black76ButterflyLong(
            test_F, test_K1, test_K2, test_K3, test_T, test_r, test_sigma
        )
        test_methods = list(butterfly.call1.get_all_greeks().keys()) + [
            "price",
        ]
...
class TestBlack76ButterflyShort:
    def test_init(self):
        # Assert K1 < K2 < K3
        with pytest.raises(AssertionError):
    ...
    def test_individual_methods(self):
        butterfly = Black76ButterflyShort(
            test_F, test_K1, test_K2, test_K3, test_T, test_r, test_sigma
        )
        test_methods = list(butterfly.put1.get_all_greeks().keys()) + [
            "price",
